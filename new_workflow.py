...
# langfuse_handler is now instantiated per invocation inside the loop
...
def slide_agent_node(state: AgentState) -> Command[Literal["supervisor"]]:
    ...
    def generate_slide(slide_number: int, instructions: str, images_url: str, style: str, color_scheme: str, design_language: str, first_slide_reference: str = "", design_attrs: dict = None) -> tuple[str, str]:
        ...
        try:
            ...
            presentation_prompt = f"""The response MUST be a full HTML document, starting with <!DOCTYPE html> and ending with </html>.
            ...
            Key Requirements:
            - Output a complete and valid HTML5 document. This means starting with '<!DOCTYPE html>' (case-insensitive) and encapsulating everything within '<html>' and '</html>' tags.
            ...
            if not is_full_html:
                logger.warning(f"Slide #{slide_number}: LLM response did not provide a full HTML document (missing DOCTYPE or html tags). Raw response snippet: {stripped_content[:200]}")
                # Heuristic check for partial HTML content
                if "<div" in stripped_content.lower() or \
                   "<p" in stripped_content.lower() or \
                   "<body" in stripped_content.lower() or \
                   "<section" in stripped_content.lower():
                    logger.info(f"Slide #{slide_number}: Attempting to wrap partial HTML content in a basic HTML skeleton.")
                    html_content_to_save = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="slide-container">
        {raw_html_content} {/* Use original raw_html_content for wrapping */}
                    ...
                    logger.warning(f"Slide #{slide_number}: Partial content does not appear to be HTML. Using raw response without wrapping. This might lead to rendering issues.")
            elif not raw_html_content.startswith("<!DOCTYPE html>"): # If it's full HTML but might have leading whitespace
                html_content_to_save = stripped_content
            # --- End of new HTML robustness logic ---
            ...
            return f"Slide #{slide_number} generated successfully. Saved to {output_path}", html_content_to_save # Return the processed content
        except Exception as e:
            tool_duration = time.perf_counter() - tool_start_time
...
# trace_id and config are now generated inside the loop
...
while True:
    ...
    # Instantiate handler and generate IDs INSIDE the loop for each invocation
    current_trace_id = str(uuid.uuid4())
    current_langfuse_handler = CallbackHandler(
        secret_key=os.getenv("LANGFUSE_SECRET_KEY"),
        public_key=os.getenv("LANGFUSE_PUBLIC_KEY"),
        host="https://cloud.langfuse.com",
        # The handler usually picks up trace_id from the config if not set directly.
    )

    current_config = {
        "recursion_limit": 100,
        "configurable": {
            "trace_id": current_trace_id, # Use the new trace_id for this specific invocation
        },
        "callbacks": [current_langfuse_handler], # Use the new handler instance
        "run_id": current_trace_id # Set the root run_id for this invocation
    }

    # Create initial state - ensure all relevant fields are reset for a new run
    initial_state = {
    ...
    try:
        logger.info(f"Starting workflow execution with trace_id: {current_trace_id}")
        # Note: The global 'config' in LLM calls within tools might need to be updated
        # if they are not receiving the 'current_config' properly.
        # LangChain's `create_react_agent` and tool invocation should propagate the config from `app.invoke`.
        result = app.invoke(initial_state, config=current_config)
        logger.info(f"Workflow execution completed for trace_id: {current_trace_id}")
        logger.debug(f"Final result for trace_id {current_trace_id}: {result}")
        ...
        for m in result["messages"]:
            if isinstance(m, ToolMessage):
                logger.debug(f"Tool message content for trace_id {current_trace_id}: {m.content}")
                # print(f"ToolMessage: {m.content}") # Avoid duplicate printing if logger is sufficient
    except Exception as e:
        logger.error(f"Error in workflow execution for trace_id {current_trace_id}: {str(e)}", exc_info=True)
        print(f"Error occurred: {str(e)}")
    finally:
        # Explicitly flush the handler at the end of processing for this input
        logger.info(f"Flushing Langfuse handler for trace_id: {current_trace_id}")
        current_langfuse_handler.flush()